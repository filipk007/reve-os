...
- `README.md` — **Revenueable-specific deployment, customizations, gotchas, and roadmap. READ THIS FIRST.**
...
## Revenueable Fork — Quick Reference

This is the Revenueable-managed fork of `ferm-the-kiln/clay-webhook-os`. See `README.md` for the full picture. Highlights:

### Deployment
- **VPS**: AWS EC2, IP `34.204.7.200`, SSH key `~/Downloads/8cpu.pem`, code at `/opt/clay-webhook-os/`
- **Backend**: `https://clay.revenueable.com` (uvicorn on port 8001, systemd service `clay-webhook-os`)
- **Dashboard**: `https://app.revenueable.com` (Next.js on port 3000, systemd service `clay-dashboard`)
- **Workers**: 10 parallel, default model `sonnet`, Claude Max subscription
- **Local daemon**: Mac LaunchAgent `com.clay-webhook-os.clay-run.plist` polls server for jobs and runs them locally

### Customizations (re-apply after upstream pulls)
- `app/config.py` — `app.revenueable.com` added to `allowed_origins`
- `app/core/context_filter.py` — extended `SKILL_CLIENT_SECTIONS` for email-gen
- `app/core/context_store.py` — slugify regex preserves leading `_` (so `_defaults` doesn't become `defaults`)
- `skills/email-gen/skill.md` + `skills/quality-gate/skill.md` — `model_tier: light`, `semantic_context: false`, `skip_defaults: true`, explicit `context:` refs

### Reverted upstream commits
- `company-qualifier` skill (we use our own pipeline)
- Google OAuth login (broken on headless VPS)
- Domain restriction middleware

### Critical gotchas
- **`client_slug` must be inside `data` object**, NOT top-level. `context_assembler.py:103` reads `data.get("client_slug")`.
- **Skill `context:` refs must be in YAML frontmatter**, not in markdown body. The body section is documentation only.
- **Restart wipes the prompt cache** (`prompt_cache` is in-memory). First call after restart is cold (~30s).
- **Use `kill -9` not `systemctl kill`** — uvicorn graceful shutdown hangs on health-poll connections.
- **VPS IP changes on restart** — needs Elastic IP attached. Update DNS A records + re-run certbot when it changes.
- **`useSearchParams()` Suspense bug** — upstream periodically introduces this in `tables/page.tsx`. Wrap in `<Suspense>`.
- **New upstream commits often need new venv deps** (e.g. `aiohttp` for the bridge endpoint). Install in `/opt/clay-webhook-os/.venv/`.

### Where this is going
The endgame is migrating Fivefox cold outbound from Clay HTTP Actions into the table system itself. Upstream's recent additions (HTTP/waterfall/lookup/script columns, bridge, enrichment wizard) make the table builder a self-contained Clay alternative. The missing piece is a `SkillColumn` type that wraps the existing skill machinery (context_assembler + claude_executor) for use inside table runs. See README.md "Where this is going" for the full plan.

### Active client
- `clients/fivefox-fintech/profile.md` — Fivefox Media (fintech performance marketing). Sections: Company, What They Sell, Target ICP, Competitive Landscape, Recent News, Value Proposition, Tone Preferences, Qualification Criteria, Campaign Angles, Angle Selection Matrix, Personas, Battle Cards, Signal Playbook, Proven Responses, Active Campaigns.
